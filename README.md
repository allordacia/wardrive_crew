...
Then open `http://<host>:8080/` on the device you want to use.

If you're driving around with a phone, hit the **GPS** button on the page
to grant location and start streaming fixes back to the box. Hit the
**MONITOR** button to flip the adapter into monitor mode and start
rotating pcaps into `./data/pcaps/`.

### Environment variables

| Variable                 | Default | Notes                                             |
|--------------------------|---------|---------------------------------------------------|
| `WARDRIVE_IFACE`         | `wlan0` | Wireless interface name on the host.              |
...
- `GET  /api/networks`  — recent BSSIDs with last-seen GPS fix.
